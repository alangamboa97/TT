...
\item Elaboración de incidencia.
\item Coneción entre el sistema de visión artificial y el sistema de administración. 
...
\subsubsection{Dispositivo Portátil}

\subsubsection{Visión Artificial}

La visión artificial constituye un dominio de la inteligencia artificial dedicado a capacitar a las computadoras y sistemas para extraer información valiosa de imágenes digitales, videos y otras fuentes visuales, con el propósito de tomar decisiones o realizar recomendaciones. Este campo depende en gran medida de la disponibilidad de datos abundantes. A través de iterativos análisis de datos, la visión artificial realiza comparaciones repetidas hasta lograr identificar discrepancias y, en última instancia, reconocer patrones visuales en las imágenes. \cite{IBMvision}.

La visión artificial se refiere a la habilidad de los sistemas computacionales para analizar y obtener información significativa a partir de imágenes y videos. Al emplear técnicas de aprendizaje profundo, las computadoras pueden interpretar las imágenes de manera análoga a la capacidad humana \cite{AP}. Este campo de estudio presenta diversas aplicaciones prácticas, algunas de las cuales incluyen: 

\begin{itemize}
  \item Moderación de contenido: eliminación automática de material inseguro o inapropiado en archivos de imágenes y videos.
  \item Reconocimiento facial: identificación de rostros y reconocimiento de características como la apertura de los ojos, la presencia de gafas y la existencia de vello facial.
  \item Clasificación de imágenes: identificación de elementos como logotipos de marcas, prendas de vestir, equipo de seguridad y otros detalles visuales en la imagen.
\end{itemize}
...
Las aplicaciones web siguen una arquitectura cliente-servidor, dividiendo su código en dos componentes principales: \emph{scripts} del lado del cliente y scripts del lado del servidor.

En la arquitectura del lado del cliente, el script se encarga de la interfaz de usuario, gestionando elementos visuales como botones y menús desplegables. Cuando el usuario accede a la aplicación web, el navegador carga el script del lado del cliente, permitiendo la visualización de elementos gráficos y texto para la interacción del usuario, como leer contenidos, ver videos o completar formularios. Las acciones del usuario, como hacer clic en botones, generan solicitudes al servidor.

En la arquitectura del lado del servidor, el \emph{script} maneja el procesamiento de datos. El servidor procesa las solicitudes del cliente y envía respuestas correspondientes. Estas solicitudes pueden implicar obtener más datos, editar existentes o guardar nuevos datos. Por ejemplo, al hacer clic en "Leer más", el servidor envía contenido adicional, mientras que al hacer clic en "Enviar", el servidor guarda los datos del usuario en la base de datos. En ocasiones, el servidor completa la solicitud y envía la página HTML completa al cliente, un proceso conocido como renderizado del lado del servidor \cite{aplicacionWeb}.

\subsubsection{\emph{Front end}}

El \emph{front end} es aquello que ven los usuarios e incluye elementos visuales, como botones, casillas de verificación, gráficos y mensajes de texto. Permite a los usuarios interactuar con la aplicación. 
...
Hay tres lenguajes de computación principales que afectan a la forma en que los usuarios interactúan con el \emph{front end}:

\begin{itemize}
\item HTML (Lenguaje de Marcado de Hipertexto): Define la estructura del \emph{front end} y los diversos elementos del DOM.
\item CSS (Hojas de Estilo en Cascada): Define el estilo de la aplicación web, abarcando aspectos como el diseño, las fuentes, los colores y el estilo visual.
\item JavaScript: Añade una capa de funcionalidad dinámica mediante la manipulación del DOM. JavaScript posibilita cambios en una página y la presentación de nueva información. Con ello, el \emph{front end} puede gestionar interacciones fundamentales de los usuarios, como visualizar un calendario o verificar la validez de una dirección de correo electrónico. Las solicitudes más complejas se trasladan al \emph{back end}.
\end{itemize}


\subsubsection{\emph{Back end}}
El \emph{back end} son los datos y la infraestructura que permiten que la aplicación funcione. Almacena y procesa los datos de las aplicaciones para los usuarios.

El \emph{back end} de la aplicación administra la funcionalidad general de la aplicación web. Cuando el usuario interactúa con el \emph{front end}, la interacción envía una solicitud al \emph{back end} en formato HTTP. El \emph{back end} procesa la solicitud y devuelve una respuesta.

Cuando el \emph{back end} procesa una solicitud, normalmente interactúa con los siguiente elementos:

\begin{itemize}
\item Servidores de bases de datos para recuperar o modificar datos relevantes
\item Microservicios que realizan un subconjunto de las tareas solicitadas por el usuario
\item API de terceros para recopilar información adicional o realizar funciones adicionales
\end{itemize}

El \emph{back end} utiliza varios protocolos y tecnologías de comunicación para completar una solicitud. Además, gestiona miles de solicitudes distintas de forma simultánea. El \emph{back end} combina técnicas de concurrencia y paralelismo, como la distribución de solicitudes en muchos servidores, el almacenamiento en caché y la duplicación de datos.


\subsubsection{\emph{End point}}
...
\begin{itemize}
\item 2 si el parpadeo es significativo.
\item 1 si el parpadeo es moderado.
\item 0 si no se detecta un parpadeo significativo.
\end{itemize}
\end{itemize}

\begin{center}
  \includegraphics[scale=0.6]{imagenes/funcionesdetec.JPG}
...
\begin{enumerate}
\item Cálculo de la Apertura de la Boca:
...
\begin{center}
  \includegraphics[scale=0.6]{imagenes/Somnolencia1.JPG}
...
\begin{itemize}
  \item Genera alertas en casos específicos, como detectar más de dos bostezos por minuto.
  \item Inicia la grabación de video en situaciones de interés, como cuando somnolencia.
\end{itemize}
...
\begin{center}
  \includegraphics[scale=0.11]{imagenes/Perifericos1.jpeg}
...
\begin{center}
  \includegraphics[scale=0.11]{imagenes/Perifericos2.jpeg}
...
\section{Pruebas}
\subsection{Módulo de Visión Artificial}

\subsubsection{Prueba de detección y clasificación de somnolencia:}
\begin{itemize}
 \item Verificar la capacidad de \texttt{dlib} para la detección de rostros, ojos y boca en diferentes condiciones de iluminación y fondos.
 \item Simular estados de apertura y cierre de ojos y boca para confirmar la precisión en la clasificación de la somnolencia.
 \item Evaluar la respuesta del sistema ante cambios rápidos en los estados faciales.
\end{itemize}
...
\begin{itemize}
  \item Verificar la estabilidad y velocidad de la conexión LTE para la transferencia de información de posicionamiento GPS.
  \item Evaluar la capacidad de recuperación en casos de pérdida temporal de conectividad.
\end{itemize} 

\subsubsection{Prueba de Posicionamiento GPS:}
\begin{itemize}
  \item Confirmar la precisión del posicionamiento proporcionado por GNSS en diferentes entornos geográficos.
\end{itemize}

\subsection{Estación Base (Amazon Amplify)}

\subsubsection{Prueba de Integración con GitHub:}
\begin{itemize}
  \item Confirmar la sincronización automática entre el repositorio en GitHub y la aplicación web en Amplify Hosting.
  \item Evaluar la eficacia del ciclo de desarrollo y despliegue.
\end{itemize}

\subsection{Gestión de Contenido Multimedia y Seguridad}

\subsubsection{Prueba de Almacenamiento en S3:}
\begin{itemize}
  \item Evaluar la velocidad de acceso y la escalabilidad del almacenamiento en la nube S3 para contenido multimedia.
  
  \item Verificar la disponibilidad y confiabilidad de los archivos almacenados.
\end{itemize}

\subsubsection{Prueba de Autenticación y Gestión de Identidades (Amazon Cognito):}
\begin{itemize}
  \item Validar la seguridad y eficacia del proceso de autenticación de usuarios.
  \item Evaluar la gestión de permisos y credenciales de acceso.
\end{itemize}

\subsection{Base de Datos (DynamoDB)}

\subsubsection{Prueba de Rendimiento de DynamoDB:}
\begin{itemize}
  \item Evaluar la velocidad de lectura y escritura de datos en DynamoDB bajo carga variable.
  \item Confirmar la escalabilidad del sistema de gestión de bases de datos.
\end{itemize}

\subsection{Aplicación Web (Amplify y React)}
...
\begin{itemize}
  \item Evaluar la velocidad de carga y la reactividad de la interfaz de usuario en diferentes dispositivos y navegadores.
\end{itemize}

\subsection{Pruebas de Integración del Sistema}

\subsubsection{Prueba de Interacción entre Módulos:}
\begin{itemize}
  \item Verificar la correcta integración y comunicación entre el módulo de visión artificial, la estación base y la base de datos.
\end{itemize}

\subsubsection{Prueba de Alertas y Monitoreo en Tiempo Real:}
\begin{itemize}
  \item Confirmar la emisión de alertas ante la detección de síntomas de somnolencia.
  \item Evaluar la visualización en tiempo real de la ubicación del conductor desde la aplicación web.
\end{itemize}
...
\begin{itemize}
  \item Realizar una prueba completa del sistema para validar su funcionalidad global en condiciones simuladas y reales.
\end{itemize}
