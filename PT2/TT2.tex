...
\subsubsection{Implementación del sistema de Geolocalización}


\textbf{Habilitar GPS y obtener datos de posici?n GPS desde la Jetson Nano \cite{Waveshare}}

Se inici? abriendo una terminal e instalando bibliotecas con los siguientes comandos.

Se remplaz? your\_user\_name con el nombre de usuario nano.
...
Se coloc? el DIP en ON y se ejecut? minicom con el comando AT para probar la SIM7600.
...
Comunicaci?n por Minicom:


Para obtener los datos de posici?n GPS, fue necesario interactuar con el m?dulo SIM7600X a trav?s del programa en Python:
...
La primer configuraci?n que se realiz?, fue la habilitaci?n y activaci?n de la red 4G Lte en la Jetson Nano por medio de la interfaz wwan0 \cite{Waveshare} , que nos permite conectarnos a internet por medio de la red celular, se utiliz? una SIM Telcel para adquirir GB y poder acceder a internet.\\
...
El siguiente comando se utiliz? para verificar la conexi?n.
...
Posteriormente se descarg? el controlador con los siguinetes comandos:
...
Se us? el permiso de root para instalar el controlador:
...
Se comprob? si se reconoce la interfaz wwan0.
...
Se realiz? la comunicaci?n por Minicom:
...
Posteriormente se realiz? la conexi?n del M?dulo Central de Procesamiento y la apliaci?n web, se utiliz? la librer?a \textbf{Boto3}\\

Boto3 es una biblioteca de Python desarrollada por Amazon Web Services (AWS) que permite interactuar con los servicios de AWS de manera program?tica. Proporciona una interfaz de programaci?n de aplicaciones (API) f?cil de usar para acceder y administrar recursos en la nube de AWS \cite{Boto3}.\\

Para crear una conexi?n a AWS Amplify utilizando Boto3, se siguieron los siguientes pasos :\\

En primer lugar, se instal? Boto3 utilizando el comando: pip install boto3. A continuaci?n, se configuraron las credenciales de AWS, proporcionando las claves de acceso de IAM que otorgan acceso a los servicios de AWS. Estas credenciales se configuraron manualmente en el archivo de configuraci?n. Luego, se import? el m?dulo Boto3 en el script de Python y se cre? una conexi?n al servicio de AWS Amplify utilizando las credenciales configuradas anteriormente. A partir de la conexi?n establecida, se pudieron utilizar los m?todos proporcionados por el cliente de Boto3 para interactuar con AWS Amplify.
...
\begin{itemize}
\item \textbf{Path:} "/\\ \textbf{Descripción:} En esta direcci?n se encontar? la el formulario para poder iniciar sesi?n o registrarse
...
Entrando a nuestra consola de AWS, en la secci?n de Amazon Cognito, se puede observar dos grupos de usuarios, uno de tipo Administrador, el cual puede realizar cambios a la configuración de la aplicaci?n, y otro de tipo de Usuario, el cual sólo puede hacer uso del sistema de inició de sesión y registro ofrecido por Cognito.
...
La funci?n de la figura \ref{getConductor}, obtiene los datos de un solo Conductor.


\begin{center}
  \includegraphics[scale=0.5]{imagenes/listconductors}
\captionof{figure}{Funci?n listConductors}
 \label{fig:listConductors} 
\end{center}

La funci?n de la figura \ref{fig:listConductors} obtiene todos los datos de todos los conductores almacenados en la base de datos.
...
La funci?n de la figura \ref{fig:getIncidencia} obtiene los datos de una sola Incidencia.
...
La funci?n de la figura \ref{fig:listIncidencias} obtiene los datos de todas las incidencias de almacenadas en la base de datos.
...
\captionof{figure}{Funci?n createConductor}
 \label{fig:createConductor} 
\end{center}

La funci?n de la figura \ref{fig:createConductor} se encarga de crear el registro de un conductor en la base de datos.
...
La funci?n de la figura \ref{fig:createIncidencia} se encarga de crear una Incidencia en la base de datos.

Para el archivo de \emph{subscriptions} se tienen la siguiente funci?n:
...
Al estar trabajando con GraphQL dentro del proyecto, la manera en que se podr?n realizar las operaciones \emph{CRUD - (Create, Read, Update, Delete)}, ser? mediante funciones JSON.
...
Posteriormente se necesita ingresar a la secci?n de consultas.
...
\subsubsection{Implementación de una red neuronal convolucional capaz de detectar ojos cerrados y abiertos}
