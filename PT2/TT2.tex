...
Para el sistema de administración se corroborará que en los casos en que el sistema de somnolencia detecte una incidencia, esta será enviada correctamente y pueda ser visualizada desde el sistema de administración. A su vez, se constará que la información de la ubicación geográfica del sujeto de prueba esté disponible en todo momento.  
...
El teorema de Shannon-Hartley establece la máxima cantidad de información que puede ser transmitida sin error con un ancho de banda específico y que está expuesto a la interferencia de ruido. La ecuación para la capacidad de canal es\cite{shannon}:
...
segundo, B es el ancho de banda en hertz, S es la potencia de la señal útil en watts y N es la potencia de ruido presente en el canal expresada en watts. Al término S/N se le conoce como relación señal a ruido.
...
Si el conductor presenta un estado de ojos cerrados por más de 5 segundos, se considerará como
un estado de somnolencia, por lo que se activará una alarma para alertar al conductor.Así mismo se evaluará el grado de apertura de la boca para determinar si el conductor bosteza, si el conductor presenta más de un bostezo dentro de un minuto el sistema lo clasificará como somnolencia. A su vez
el sistema tomará un videoclip dónde se aprecie el momento en que el conductor presentó un estado de somnolencia.
...
\end{tabular}} \\ \hline
\multicolumn{2}{|l|}{\begin{tabular}[c]{p{0.7\linewidth}@{}l@{}}\textbf{Solución del Requerimiento:}\\
El sistema obtendrá la imagen del rostro y en base a esta se obtendrán las imágenes de la boca y ojos del conductor detectados por dicho método.  
\end{tabular}}                                                    \\ \hline
\end{tabular}
...
Por lo anterior, se decidió a utilizar el estándar LTE-M. 
...
Dentro de los SGBD NoSQL se eligió MongoDB, ya que la aplicación  como base el lenguaje JavaScript y  Node.js para realizar el \emph{backend} de la aplicación, por lo cual MongoDB complementa y facilita el desarrollo del proyecto en herramientas basados en JavaScript. Además, MongoDB es de código abierto, y es una herramienta gratuita hasta cierto punto, por lo cual no se requerirá una licencia para la implementación\cite{mongo}. Otra característica importante es que es una base de datos multiplataforma, esto para que se realice información desde el sistema operativo Linux, que será utilizado en la Nvidia Jetson Nano.
...
\begin{center}
  \includegraphics[scale=.3]{imagenes/lteantena}
...
\subsubsection{Diseño de una red neuronal convolucional capaz de detectar ojos cerrados y abiertos}
\subsubsection{Entorno de desarrollo en la NVIDIA Jetson Nano}
...
Como primer paso, se descargó el \emph{JetPack SDK 4.6.3} \cite{JetPackSDK},el cual se refiere a un archivo de imagen del sistema operativo específicamente diseñado y optimizado para ser utilizado en la placa de desarrollo NVIDIA Jetson Nano, la imagen incluye un sistema operativo Linux, controladores de hardware específicos para la Jetson Nano y una configuración predefinida. Para el presente proyecto se utilizó la última versión disponible para la Jetson Nano 4GB, JetPack SDK 4.6.3 \cite{JetPackSDK}.\\
...
Para iniciar e interactuaar con el kit de desarrollo, se reqquirió conectar un mouse, pantalla, teclado y una fuente de alimentación, cabe mencionar que estos elementos no se incluyen en con la compra del kit de desarrollo Jetson Nano. Posteriormente, se realizó la configuración inicial del sistema operativo, el cual incluyó los siguientes pasos:
...
\item Selecionar el tamaño de partición de la aplicación, se utilizó el tamaño máximo sugerido.
\end{itemize}

\textbf{Intalación de librerias y dependencias}\\

Después de haber iniciado la Jetson Nano, se abrió una terminal y se ingresaron los siguientes comandos para la instalación de las librerias a utilizar:
...
Se importaron las librerias con Python3 para verificar que hayan sido correctamente instaladas.
\begin{center}
...
Comandos utilizados para configrar memoria virtual:
...
El comando gedit abre un archivo el cual se debe modificar añadiendo la siguiente linea y posteriormente guardando los cambios realizados. 
\begin{verbatim}
/mnt/4G.swap	none	swap	sw	0	0
\end{verbatim}

Luego, se verifico el tamaño de todos los intercambios:
\begin{verbatim}
free -h
\end{verbatim}
...
Por defecto la versión del JetPack SDK 4.6.3 viene instalado con OpenCV 4.1.1 sin CUDA, el cual es un marco que permite la computación paralela en la GPU. Para activar CUDA se utilizó el sript proporcionado en el repositiorio de github \cite{CUDA} que ademas instala la version 4.5 de OpenCV:
...
El archivo se modificó colocando la variable JOBS=4 despues de haber modificado el archivo de intercambio (\textit{swapfile}), posteriormente se corrió el archivo.
\begin{verbatim}
...
\subsubsection{Implementación de puntos faciales para detectar el nivel de apertura de los ojos y boca}



se definió una función 

\subsubsection{Detección de somnolecia}
...
\end{center} 
\subsubsection{Integración del subsistema}
...
Se coloca el DIP en ON y se ejecut? minicom con el comando AT para probar la SIM7600.
...
Ejemplos de GPS: Se conect? la antena GPS y se coloc? el receptor en un lugar abierto. Se ejecutaron los siguientes comandos para probar el GPS.
...
Para obtener los datos de posici?n GPS, fue necesario interactuar con el módulo SIM7600X a través del programa en Python:

\begin{center}
  \includegraphics[scale=0.62]{imagenes/GPSpy1.JPG}
\captionof{figure}{C?digo que permite obtener la posici?n GPS parte 1}
 \label{fig:GPSpy1} 
\end{center} 

\begin{center}
  \includegraphics[scale=0.62]{imagenes/GPSpy2.JPG}
\captionof{figure}{C?digo que permite obtener la posici?n GPS parte 2}
 \label{fig:GPSpy2} 
\end{center} 

\begin{center}
  \includegraphics[scale=0.62]{imagenes/GPSpy3.JPG}
\captionof{figure}{C?digo que permite oibtener la posici?n GPS parte 3}
 \label{fig:GPSpy3} 
\end{center} 

\begin{center}
  \includegraphics[scale=0.7]{imagenes/GPSpy4.JPG}
\captionof{figure}{C?digo que permite obtener la posici?n GPS parte 4}
 \label{fig:GPSpy4} 
\end{center} 

\textbf{AWS Maps}\\

AWS Maps es un servicio de Amazon Web Services que proporciona capacidades de mapas y geolocalizaci?n en la nube. Permite a crear, visualizar y analizar datos geoespaciales en aplicaciones y servicios.\\

Se cre? un mapa en AWS Maps donde se configur? la apariencia y funcionalidad del mismo, incluyendo los estilos de visualizaci?n, las capas de datos y las interacciones del usuario. El prop?sito de este mapa es mostrar informaci?n geogr?fica, como ubicaciones, rutas, geovallas, puntos de inter?s, entre otros. La configuraci?n del mapa permiti? personalizar su aspecto y adaptarlo a las necesidades del proyecto.\\

Para crear el mapa se inici? sesi?n en la Consola de administraci?n de AWS. Luego,se accedi? a la p?gina de AWS Maps Service en la consola de administraci?n. A continuaci?n, se hizo clic en "Crear mapa" para comenzar a crear el mapa.En el proceso de creaci?n, se seleccion? un nombre para el mapa y se especific? la regi?n de AWS donde se deseaba crearlo. Seguidamente, se eligi? el estilo de mapa y posteriormente  se configurar?n las capas del mapa, pudiendo agregar capas para mostrar datos como carreteras, edificios, r?os, y tambi?n pudo agregar sus propios datos geoespaciales como capas personalizadas, finalmente, se  de creo el mapa.\\

Una vez que creado el mapa en AWS Maps, se agrego y configuro un rastreador para realizar el seguimiento del dispositivo llamado "mydevice" en el mapa. Los rastreadores se encargaron de recopilar y actualizar la ubicaci?n de los dispositivos, brindando informaci?n en tiempo real.\\

A continuaci?n, se presenta un resumen de c?mo se cre? un rastreador en AWS Maps:\\

En la p?gina del mapa en la consola de administraci?n de AWS Maps, se hizo clic en "Crear rastreador". Se seleccion? un nombre para el rastreador y se especific? la regi?n de AWS donde se deseaba crear. Se procedi? a configurar los detalles del rastreador, como la frecuencia de actualizaci?n de la ubicaci?n de los dispositivos en este caso cada 10m env?a las coordenadas. Finalmente, se hizo clic en "Crear rastreador" para finalizar la configuraci?n del mismo.
...
\textbf{Ubicaci?n geogr?fica en la Aplicaci?n web}\\

Para obtener el historial de posiciones de un dispositivo, se defini? la funci?n llamada "getDevicePosition". Posteriormente, se utiliz? la funci?n "client.getDevicePositionHistory" con el argumento "params" para solicitar el historial de posiciones del dispositivo. Despu?s, se cre? la variable "tempPosMarkers" para almacenar los resultados obtenidos al mapear los elementos de "data.DevicePositions". Para cada elemento "devPos" en "data.DevicePositions", se imprimi? su posici?n en la consola y se gener? un nuevo objeto con propiedades "index", "Long" y "lat", que representaban los valores de la posici?n del dispositivo. Este archivo se guard? con la extensi?n .jsx.

\begin{center}
  \includegraphics[scale=0.35]{imagenes/RealTimeMap}
\captionof{figure}{C?digo para obtener la ubicaci?n geogr?fica en la aplicaci?n web}
 \label{fig:RealTimeMap} 
\end{center} 

Obtenci?n de coordenadas geogr?ficas desde la Jetson Nano 

\begin{center}
  \includegraphics[scale=0.53]{imagenes/Mymapaws2.png}
\captionof{figure}{Coordenadas geogr?ficas en la Jetson Nano}
 \label{fig:Mymapadet} 
\end{center} 
...
\subsubsection{Realizar la conexión de la aplicación web con el Módulo Central de Procesamiento}
La primer configuraci?n que se realiza, fue la habilitación y activación de la red 4G Lte en la Jetson Nano por medio de la interfaz wwan0 \cite{Waveshare} , que nos permite conectarnos a internet por medio de la red celular, se utiliza una SIM Telcel para adquirir GB y poder acceder a internet.\\

Se abri? una terminal y se ejecutaron los siguinete comandos para abrir minicom por comando:
...
El siguiente comando se utiliza para verificar la conexi?n.

\begin{center}
  \includegraphics[scale=0.9]{imagenes/4glte2.JPG}
\captionof{figure}{Verificar conexi?n.}
 \label{fig:4glte2} 
\end{center} 
...
Conexi?n a AWS Amplify utilizando Boto3
...
Para poder utilizar los servicios de Amazon Amplify, sebe estar en el  directorio raíz del proyecto y ejectutar el siguiente comando:
...
El comando mostrado en la figura \ref{fig:acli} permitirá utilizar la terminal de nuestro computador para poder interactuar con los servicios de AWS Amplify.

Para indicar a Amplify que estaremos utilizando una API en el backend, se ejecuta el siguiente comando: 
...
La consola de AWS requerirá que se introduzcan parámetros de configuración como el tipo de lenguaje de consultas a utilizar, en este caso, será GraphQL.

\begin{center}
  \includegraphics[scale=0.4]{imagenes/graphql}
\captionof{figure}{Configuración de parámetros de GraphQL}
 \label{fig:graphqlcli} 
\end{center} 

Como resultado, se tiene un endpoint provisionado por AWS Amplify el cuál utilizaremos para realizar las operaciones necesarias en la API previamente creada.


\subsubsection{Conexión Backend con DynamoDB}

DynamoDb es un servicio de base de datos NoSQL ofrecido en la misma suite de AWS Amplify.


\subsubsection{Implementación de sistema de acceso con credenciales}
...
\captionof{figure}{Funci?n getConductor}
...
\captionof{figure}{Funci?n getIncidencia}
...
\captionof{figure}{Funci?n listIncidencias}
...
\captionof{figure}{Funci?n updateConductor}
 \label{fig:updateConductor} 
\end{center}

La funci?n de la figura \ref{fig:updateConductor} se encarga de modificar datos del registro de un conductor.


\begin{center}
  \includegraphics[scale=0.5]{imagenes/deleteconductor}
\captionof{figure}{Funci?n deleteConductor}
 \label{fig:deleteConductor} 
\end{center}

La funci?n de la figura \ref{fig:deleteConductor} se encarga de eliminar un conductor de la base de datos.


\begin{center}
  \includegraphics[scale=0.5]{imagenes/createIncidencia}
\captionof{figure}{Funci?n createIncidencia}
...
\captionof{figure}{Funci?n oncreateIncidencia}
 \label{fig:oncreateIncidencia} 
\end{center}	

La funci?n de la figura \ref{fig:oncreateIncidencia} se encarga de realizar una consulta cada vez que una Incidencia nueva es dada de alta en la base de datos.
...
Para comprobar que la API permite dichas operaciones, se debe ingresar a la consola de AWS y dirigirse a la secci?n de AWS AppSync. 
...
AWS permite elegir si realizar un \emph{query}, \emph{mutation}, o \emph{subscription} mediante c?digo JSON
...
Para poder alojar y desplegar la aplicaci?n web en un servicio de \emph{hosting}, se utilizar? Amplify Hosting \cite{AwsAmplify}.	
...
Hosting requiere alg?na fuente de alojamiento de c?digo para poder desplegar la apliaci?n web. En este caso, la aplicaci?n web fue alojada utilizando \emph{Github}, por lo tanto, se selecciona esta opci?n.

Posteriormente, se requiere seleccionar el repositorio el cual ser? alojado, en este caso, el nombre de dicho repositorio es \emph{eb}. A su vez, se selecciona la ramificaci?n \emph{main} para ser desplegada.

\begin{center}
  \includegraphics[scale=0.5]{imagenes/repo}
\captionof{figure}{ramificaci?n de repositorio de Github}
 \label{fig:libraries} 
\end{center} 
...
\bibitem{Referencian4} Mell, P., \& Grance, \emph{The NIST definition of Cloud Computing}, NIST special publication, 2011
...
\bibitem{shannon} Gianluca Cornetta, \emph{Compromiso entre Modulación y Codificación}, Universidad San Pablo, Dep. de Ingeniería de Sistemas de Información y Telecomunicación
...
\bibitem{EstadodelAarte4} Mayank Chauhan, Mukesh Sakle, \emph{Study \& Analysis of Different Face Detecion Techniques}


\bibitem{EstadodelAarte5} Rodriguez, A., \& Luis, D. J. (2021, mayo). Diseño e implementación de sistema de visión artificial para alerta y detección de somnolencia mediante aprendizaje profundo aplicable en conductores de vehículos. https://dspace.unitru.edu.pe/handle/UNITRU/16872


\bibitem{EstadodelAarte6} M. Limaquispe y E. Sánchez. "SISTEMA DE DETECCIÓN DE SOMNOLENCIA MEDIANTE INTELIGENCIA ARTIFICIAL EN CONDUCTORES DE VEHÍCULOS PARA ALERTAR LA OCURRENCIA DE ACCIDENTES DE TRÁNSITO"". Repository. http://repositorio.unh.edu.pe/handle/UNH/2327

\bibitem{EstadodelAarte7} Eugenio, B. L. H., Alejandra, G. P. K., \& Pichardo Iniesta José Alejandro. (2019, junio). Diseño de un sistema electrónico para detectar somnolencia en automovilistas por medio de la actividad ocular. https://tesis.ipn.mx/handle/123456789/27287

\bibitem{EstadodelAarte8} Becerril, E. L., Ramírez, A. M., \& Velázquez, E. T. J. (s.f.). Sistema para la detección del estado de somnolencia en seres humanos, con reconocimiento de patrones. $https://uptexcoco.edomex.gob.mx/sites/uptexcoco.edomex.gob.mx/files/files/2020/articulos-Tesis/sistema_somnolencia.pdf$

\bibitem{EstadodelAarte9} Solís, H. F. (2022, 13 de septiembre). Sistema de detección de somnolencia. https://riull.ull.es/xmlui/bitstream/handle/915/30312/Sistema%20de%20deteccion%20de%20somnolencia.pdf?sequence=1&amp;isAllowed=y

\bibitem{EstadodelAarte10} Pacay, C., \& Blanca Fanny. (2020). Desarrollar un prototipo de reconocimiento facial basado en Machine Learning para detectar estado de Somnolencia en conductores de una cooperativa de transporte. http://repositorio.ug.edu.ec/handle/redug/49449?mode=full

\bibitem{EstadodelAarte11} E. L. Benavides y M. M. Medina. "Sistema Basado en la Detección y notificación de somnolencia para conductores de autos". Repositorio Universidad de Córdoba. https://repositorio.unicordoba.edu.co/bitstream/handle/ucordoba/536/Trabajo%20de%20grado.pdf?sequence=1&amp;isAllowed=y

\bibitem{EstadodelAarte12} Adrián, Ñ. H. (2022, 30 de junio). Detección de somnolencia para conducción sin accidentes. http://hdl.handle.net/10045/124695

\bibitem{EstadodelAarte13} Avila, R., \& Arturo, W. (2016). Geolocalización transaccionalidad mapa ecuador cne democracia localización ubicación. http://repositorio.ug.edu.ec/handle/redug/16806

\bibitem{EstadodelAarte14} Vilca, G., \& Eduardo, J. (2016, 3 de agosto). Diseño e implementación de un sistema de geolocalización en interiores para plataforma Android via la red Enterprise WLAN de la PUCP. https://tesis.pucp.edu.pe/repositorio/handle/20.500.12404/7156

\bibitem{EstadodelAarte15} Federico, A., Guillermo, C., \& Guzmán, P. (2018). Geolocalizacoón con LoRa mediante multilateración. https://www.colibri.udelar.edu.uy/jspui/handle/20.500.12008/20293

\bibitem{EstadodelAarte16} Ramírez, C. E. E., Flores, R. E. R., \& Hernández, J. A. V. (2018). Propuesta de un sistema de geolocalización y monitoreo vía GPS/GSM/GPRS aplicado a un pulsómetro para personas con enfermedades vasculares. https://tesis.ipn.mx/handle/123456789/27690

\bibitem{EstadodelAarte17} Espinoza, B. R. A. V. (2017). Influencia de un sistema de geolocalización en el control y monitoreo de vehículos con dispositivos GPS en una empresa logística.


\bibitem{EstadodelAarte18} Juan Carlos, C. P. (2017, 15 de febrero). Diseño de la red de acceso LTE en el distrito de Jesús María ¡https://tesis.pucp.edu.pe/repositorio/handle/20.500.12404/7847
...
\bibitem{mor} Chandrakant Chandewar, Ashlesha Singh, \emph{Driver Drowsiness Alert System with Effective 
Feature Extraction}, International Journal for research in emergin science and technology, 2018

\bibitem{tracking} Quectel, \emph{Lte Standard- GNSS Application Note}, LTE Standard Sixfab Module Series, 2019
...
\bibitem{JetPackSDK} nvidia. "JetPack SDK 4.6.3". NVIDIA Developer. https://developer.nvidia.com/jetpack-sdk-463 (accedido el 17 de junio de 2023).

\bibitem{JetStats} Kangalow. "Jtop: The ultimate tool for monitoring NVIDIA jetson devices - jetsonhacks". JetsonHacks. https://jetsonhacks.com/2023/02/07/jtop-the-ultimate-tool-for-monitoring-nvidia-jetson-devices/ (accedido el 17 de junio de 2023).

\bibitem{SwapFile} N. Escobar. "Para qué sirve el Swap en Linux y cómo cambiarlo". Hipertextual. https://hipertextual.com/2015/09/swap-en-linux (accedido el 19 de junio de 2023).

\bibitem{Tensorflow} nvidia. "JetPack SDK 4.6.3". NVIDIA Developer. https://developer.nvidia.com/jetpack-sdk-463 (accedido el 17 de junio de 2023).

\bibitem{CUDA} Nakai-Omer. "GitHub - mdegans/nano\_build\_opencv: Build opencv on nvidia jetson nano". GitHub. https://github.com/mdegans/nano\_build\_opencv (accedido el 20 de junio de 2023).
