...
\begin{center}
  \includegraphics[scale=0.2]{imagenes/JNmicrosd}
\captionof{figure}{Ranura para tarjeta microSD para almacenamiento}
 \label{fig:JNmicrosd} 
\end{center}
...
\begin{center}
  \includegraphics[scale=0.4]{imagenes/ReRoCNN.png}
...
\item Detección del rostro: Se convierte cada cuadro a escala de grises utilizando la función cv2.cvtColor() y se utiliza el modelo face\_model para detectar los rostros en la imagen. Luego, para cada rostro detectado en la imagen, se detectan los landmarks o puntos faciales utilizando el modelo de landmark\_model previamente definido, finalemnte los  puntos faciales detectados se convierten en un arreglo NumPy.
...
\item Marcado del labio: Se dibujan contornos alrededor del labio superior e inferior utilizando la función cv2.drawContours() en cada cuadro.
...
\item Cálculo de la distancia del labio: Se llama a la función cal\_yawn( ) para calcular la distancia del labio, que representa la apertura de la boca y se imprime.
...
\item Detección de límite de apertura de la boca: Se compara la distancia del labio calculada con un umbral predefinido (yawn\_thresh) para determinar si se ha superado el límite de apertura de la boca. Si es así, se muestra.
