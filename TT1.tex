...
A1=(m1*lc1^2+m2*l1^2+m2*lc2^2+2*m2*l1*lc2*cos(q2)+I1+I2);
A2=(m2*lc2^2+m2*l1*lc2*cos(q2)+I2);
A3=-(2*m2*l1*lc2*sin(q2))*q1p*q2p;
A4=-(m2*l1*lc2)*sin(q2)*q2p^2;
A5=(m1*lc1+m2*l1)*g*sin(q1);
A6=m2*g*lc2*sin(q1+q2);


q1pp=(Tau1-(A2*q2pp+A3+A4+A5+A6))/A1;

salidap1=q1pp;
\end{lstlisting}

Para el bloque de planta para el eslabón 2:

\begin{lstlisting}
function salidap2=planta2(a)
Tau2=a(1);
q1=a(2);
q2=a(3);
q1p=a(4);
...
B1=(m2*lc2^2+m2*l1*lc2*cos(q2)+I2);
B2=(m2*lc2^2+I2);
B3=(m2*l1*lc2)*sin(q2)*q1p^2;
B4=(m2*g*lc2)*sin(q1+q2);

q2pp=(Tau2-(B1*q1pp+B3+B3+B4))/B2;

salidap2=q2pp;
\end{lstlisting}

El bloque utilizado para el control (Tanto en el modelo de Simulink como SimMechanics):

\begin{lstlisting}
function output=control_c(in)
q1=in(1);
q1p=in(2);
q2=in(3);
q2p=in(4);
...
A3=-(2*m2*l1*lc2*sin(q2))*q1p*q2p;
A4=-(m2*l1*lc2)*sin(q2)*q2p^2;
A5=(m1*lc1+m2*l1)*g*sin(q1)+m2*g*lc2*sin(q1+q2);
...
Vaux1=q1ppd+Kd*(q1pd-q1p)+Kp*(q1d-q1);
Vaux2=q2ppd+Kd*(q2pd-q2p)+Kp*(q2d-q2)
Vaux=[Vaux1;Vaux2];
...
%%%PD+Control
% Tau=Kp*(qpd-qp)+Kp*(qd-q)+M*qppd+C*qpd+G;
%
% Tau1=Tau(1,:);
% Tau2=Tau(2,:);
...
El bloque correspondiente a la cinemática inversa:
\begin{lstlisting}
function cine_inversa=CinematicaInversa_2GDL_c(a)
xd=a(1);
yd=a(2);
l1=.3;
l2=.175;
q2=-2*atan2(sqrt((l1+l2)^2-(xd^2+yd^2)),sqrt((xd^2+yd^2)-(l1-l2)^2));
q1=atan2(yd,xd)-atan2((l2*sin(q2)),(l1+(l2*cos(q2))))+(pi/2);
cine_inversa=[q1 q2];
\end{lstlisting}

El bloque correspondiente a la trayectoria deseada:
\begin{lstlisting}
thetad=[0:0.1:10];
a=.4;
b=.2;
...
El código que se usa para graficar los eslabones siguiendo la trayectoria en xy:
\begin{lstlisting}
function salida=grafica_2GDL_c(u)
%recibo datos
theta1=u(1);
theta1p=u(2);
theta2=u(3);
...
xm=l1*sin(theta1);
ym=-l1*cos(theta1);
xm1=l1*sin(theta1)+l2*sin(theta1+theta2);
ym1=-l1*cos(theta1)-l2*cos(theta1+theta2);

figure(1)
plot([0,xm],[0,ym],'-bo')
hold on
plot([xm,xm1],[ym,ym1],'-ro')
hold on
plot(xdes,ydes);
hold off
...
\includepdf[pages=-]{planos_finales_SCARA}

\includepdf[pages=-]{joined_document}
...
\includepdf[pages=-]{Nuevo_documento_datasheets}
